...
pygame.display.set_caption('Welcome to Memory Match!')
...
def draw_backgrounds():
    top_menu = pygame.draw.rect(screen, babypink, [0, 0, WIDTH, 100])
    title_text = title_font.render('Memory Matcher <3!', True, white)
    screen.blit(title_text, (10, 20))
    board_space = pygame.draw.rect(screen, hotpink, [0, 100, WIDTH, HEIGHT - 200], 0)
    bottom_menu = pygame.draw.rect(screen, babypink, [0, HEIGHT - 100, WIDTH, 100], 0)
    restart_button = pygame.draw.rect(screen, hotpink, [10, HEIGHT - 90, 200, 80], 0, 5)
...
def draw_board():
    ...
    for r in range(rows):
        for c in range(cols):
            if correct[r][c] == 1:
                pygame.draw.rect(screen, gold, [c * 75 + 10, r * 65 + 110, 54, 54], 3, 4)
                piece_text = small_font.render(f'{spaces[c * rows + r]}', True, babypink)
...
while running:
    ...
    if matches == rows * cols // 2:
        game_over = True
        winner = pygame.draw.rect(screen, hotpink, [10, HEIGHT - 300, WIDTH - 20, 80], 0, 5)
    ...
    if first_guess:
        piece_text = small_font.render(f'{spaces[first_guess_num]}', True, pink)
    ...
    if second_guess:
        piece_text = small_font.render(f'{spaces[second_guess_num]}', True, pink)
